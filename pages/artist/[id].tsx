    ...
    return (
      <div className="min-h-screen bg-[#0d0d0d] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/artists" className="px-6 py-3 bg-amber-400 text-gray-900 rounded-xl text-sm font-bold hover:bg-amber-300 transition-all">
            Browse Artists
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ee]">
        ...
        {profile.artSubForms?.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {profile.artSubForms.map((s) => (
              <span key={s} className="px-3 py-1 bg-violet-100 text-violet-700 text-xs font-semibold rounded-full">{s}</span>
            ))}
          </div>
        )}
      </div>
          ...
          {profile.experience && (
            <div className="flex-shrink-0">
              <p className="text-3xl sm:text-4xl font-black text-white leading-none">
                {profile.experience.replace("More than ", "").replace(" years", "").replace("Less than 1 year", "<1")}
              </p>
      ...
      <div className="px-5 sm:px-10 lg:px-14 py-10 space-y-12">
        ...
        {images.length > 0 && (
          <div>
            <div className="mb-5"><Label>Performance Photos</Label></div>
        ...
        {videos.length > 0 && (
          <div>
            <div className="mb-5"><Label>Videos</Label></div>
            <AutoVideoCarousel urls={videos} captions={profile.youtubeVideoCaptions} />
          </div>
        )}

        {/* ── Contact / Sign-in gate ── */}
        <div>
          <Label>Connect</Label>

          {!isLoggedIn && hasContact ? (
            /* Locked contact card for anonymous visitors */
            <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6 flex flex-col sm:flex-row items-center gap-5">
              ...
              <div className="flex-1 text-center sm:text-left">
                <p className="font-black text-gray-900">Contact info is hidden</p>
                <p className="text-sm text-gray-500 mt-0.5">Sign in to view phone number, email and book this artist</p>
              </div>
              <Link
                href={{ pathname: "/signup", query: { role: "client", returnTo: router.asPath } }}
                className="px-6 py-3 bg-amber-400 text-gray-900 rounded-xl font-black text-sm hover:bg-amber-300 transition-all shadow-md shadow-amber-400/20 whitespace-nowrap"
              >
                Sign in to contact
              </Link>
            </div>
          ) : (
            /* Full contact cards for logged-in users */
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              {profile.instagram && (
                <a href={profile.instagram} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-4 bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center gap-7">
            ...
            <div className="flex flex-wrap gap-3">
              {isLoggedIn ? (
                ...
                <Link
                  href={{ pathname: "/signup", query: { role: "client", returnTo: router.asPath } }}
                  className="flex items-center gap-2 px-7 py-3.5 bg-amber-400 text-gray-900 rounded-xl font-black text-sm hover:bg-amber-300 transition-all shadow-lg shadow-amber-400/30 hover:-translate-y-0.5"
                >
                  Sign in to Book
                </Link>
              )}
